...
export class CreaterideComponent {
  ...
  initMap() {
    if (!google?.maps) {
      this.toaster.error('Google Maps failed to load');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        const defaultPlace = { lat: 22.5744, lng: 88.3629 };
        this.map = new google.maps.Map(
          document.getElementById("map") as HTMLElement,
          {
            zoom: 7,
        ...
        this.toaster.warning('Unable to get current location, using default');
      }
    );
  }
